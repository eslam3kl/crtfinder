...
print(colored("			\____/_|   \__\/    |_|_| |_|\__,_|\___|_|v2					", "red", attrs=['bold']))   
print(" ")
...
for line in all_subdomains: 
	before = line.split(word)[0] 
	after = line.split(word)[1]
	try:
		print(before + colored(word, 'red') + after) 
	except: 
		pass
output_file = word + ".txt"

if len(all_subdomains) > 1: 
	print(colored("\n[+] Founded: ", "green") + colored(str(len(all_subdomains)) + " domain related to " + word, "white", attrs=[]))
	print(colored("[+] Output file name: ", "green") + colored(output_file + "\n", "white", attrs=[]))
