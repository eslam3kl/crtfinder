...
print(" ")
print(colored("				     Coded by Eslam Akl 						   		   	", 'yellow',
              attrs=['bold']))
...
def get_user_input():
    ...
    if not options.target_url:
        ...
        print(colored("[+] ", 'red') + colored("Getting the results from crt.sh with these keywords:", 'cyan',
                                               attrs=['bold']) + colored(
            " *." + target_word + ".com / " + target_word + ".*", "white"))
        print(colored("[+] ", 'red') + colored("Start collecting info for: ", "cyan", attrs=["bold"]) + colored(
            target_word + "\n", 'white', attrs=[]))
...
def filter_crt(content_array):
    ...
    for line in content_array:
        ...
        if (line not in subdomains and word in line and " " not in line
                and '@' not in line):
            if '*' in line:
                line = line.split("*.")[1]
...
if len(all_subdomains) > 1:
    print(colored("\n[+] Founded: ", "red") + colored(str(len(all_subdomains)) + " domain related to " + word, "white",
                                                      attrs=[]))
